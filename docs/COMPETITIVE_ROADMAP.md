...
| # | Capability | Incumbent | Us today | Build | Data / vendor + cost | Eff | Pri |
|---|---|---|---|---|---|---|---|
| C1 | **EVM behavioral wallet clustering** — group many addresses to one actor via common-funding (same gas-funder), deposit-reuse, nonce/gas-price fingerprint, temporal co-activity, contract-deploy ownership, Safe-owner graph (have `safe_ownership_detector`) | TRM custom entities, Chainalysis, Arkham | only co-occurrence (`address_clustering.py`, low conf) | clustering engine + confidence | internal compute (archive node helps) | L | **P1** |
