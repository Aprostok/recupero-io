...
| `RECUPERO_DATABASE_URL` | unset (falls back to `DATABASE_URL`) | str | psycopg DSN | SaaS | Postgres DSN the `/v2` platform DAO connects with (orgs/users/keys/usage + the tenant-scoped queue). Rides the Supabase transaction pooler; a process-wide `psycopg_pool` is the drop-in scale upgrade. With genuine RLS (migration 043) this should be a **restricted, non-owner, `NOBYPASSRLS`** role — `current_principal` sets `app.current_org` on it per request so RLS scopes every query. |
| `RECUPERO_AUTH_DATABASE_URL` | unset (falls back to `RECUPERO_DATABASE_URL`) | str | psycopg DSN | SaaS | DSN for the intentionally cross-tenant, PRE-auth lookups (`signup`, `login`, API-key resolution, `accept-invite`, Stripe webhook) — these must read/write across orgs, so in prod this is the **service role with `BYPASSRLS`** (same role the worker/cron use). Defaults to the tenant DSN for single-role dev/back-compat (RLS not FORCED). See `docs/RLS_ENFORCEMENT_PLAN.md`. |
