...
| `RECUPERO_WAVE_CANCEL_GRACE_SEC` | `0` (off) | float | `[0, 120]` | audit | Grace period granted to ALREADY-RUNNING BFS wave nodes after a wave deadline cancels the queued ones, so their transfers + evidence receipts land before the shared chain adapter closes. Default 0 keeps the contract that a wave returns PROMPTLY once the deadline elapses (the #253 production hang fix — one depth-1 whale wave ran 90+ min unbounded). Set 10-30 on a whale run to trade a bounded extra wait for fewer abandoned nodes: each abandoned node loses BOTH its transfers and its evidence receipts (72 receipts lost on a real Ronin run). Either way the case is now stamped `partial_wave_deadline`, never `complete`, when any frontier node was dropped. Clamped to 120s; bad/blank → 0. |
| `RECUPERO_PREFER_ALCHEMY` | unset (off) | bool | `1/true/yes/on` | data | Route EVM transfer queries through **Alchemy** (higher quota + better pagination on chatty wallets → deeper reach) with automatic Etherscan fallback, instead of Etherscan-only. Requires `ALCHEMY_API_KEY`; a supported chain (else silently Etherscan-only). This is the ONLY way to enable Alchemy on the async `/v2` worker path (there is no `--prefer-alchemy` CLI flag there). Default off → Etherscan-only, byte-identical to prior behavior. Wires the built-but-dormant `DualBackendClient` (RIGOR-Jacob B) into `EvmAdapter`. |
...
| `RECUPERO_ASSISTANT_ENABLED` | unset (off) | bool | `1`/`true`/`yes`/`on` | v0.43 | Enables the `/v2/assistant/chat` crypto-safety AI assistant (`platform.assistant`). Default off — each turn costs an Anthropic API call and needs `ANTHROPIC_API_KEY`. Returns 503 when unset. Replies are grounded in offline screening; never gives financial/legal advice or recovery promises. |
| `RECUPERO_ASSISTANT_MODEL` | `claude-sonnet-4-6` | str | Anthropic model id | v0.43 | Model backing the assistant. A fast model is the default (chat is short-turn, cost-sensitive); pin `claude-opus-4-8` for higher-stakes deployments. |
...
| `RECUPERO_MISTTRACK_AUTO` | unset (off) | bool | `1`/`true`/`yes`/`on` | data | Opt-in: auto-run MistTrack attribution enrichment on each completed case (`worker.pipeline._maybe_misttrack_enrich`). **DOUBLY gated** — a no-op unless BOTH this flag AND `MISTTRACK_API_KEY` are set. MistTrack is PAY-PER-LOOKUP, so this defaults OFF so auto-charging never happens by accident; without it, operators still enrich on demand via `recupero-ops misttrack-enrich`. Results land as LOW-confidence `pending_review` candidates (never auto-promoted). |
| `RECUPERO_MISTTRACK_MAX_ADDRS` | `25` | int | `>= 0` | data | Per-case cap on the number of PAID MistTrack lookups when `RECUPERO_MISTTRACK_AUTO` is on (cost control; only a case's UNLABELED addresses are candidates, capped at this many). `0` disables auto-enrichment; bad/blank keeps 25. |
