...
| `RECUPERO_LENDING_LEADS` | unset (off) | bool | `1`/`true`/`yes`/`on` | v0.41 | Opt-in Aave V3 cross-address withdrawal leads (`trace.lending_runner.run_lending_leads`). When set, each traced wallet's Aave V3 Pool `Withdraw` events are fetched (indexed-user topic filter) and every withdrawal sent DIRECTLY to a different address - an exit emitted by the aToken contract, invisible to outflow enumeration - becomes a lead in `lending_leads.json` + a guarded trace-report section. Both addresses are protocol-stamped indexed topics (high confidence). Back-to-self withdrawals are context only. Leads are never a followed destination; recoverable total unchanged. Default off = zero cost. |
