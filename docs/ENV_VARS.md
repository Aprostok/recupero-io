...
| `RECUPERO_DEEP_REACH` | unset (off) | bool | `{1,true,yes,on}` to enable | v0.35.4 | **Master switch** for cold-case "go as deep as possible" tracing — turns on the whole deep-reach recipe at once instead of separate knobs: value-trace + 1:N split/peel follow + labeled mixer/exchange/bridge terminals + dormancy-aware window (=0) + (v0.36.0) cryptographic cross-chain **bridge confirmation** (`RECUPERO_BRIDGE_CONFIRM`). Only fills knobs that are NOT individually set — any explicit per-knob env var (or the `value_trace` arg) still wins, so you can `RECUPERO_DEEP_REACH=1` but pin one knob off (e.g. `RECUPERO_VALUE_TRACE_WINDOW_HOURS=168`, or `RECUPERO_BRIDGE_CONFIRM=0` for a cheaper same-chain-only deep pass). Default OFF ⇒ every existing trace (incl. Zigha 4/4) is byte-identical. **This is the recommended production-worker setting** for real bridge/multi-hop cases (Lazarus/Ronin-class dormant laundering, and any Ethereum→L2→bridge→Solana shape) — it is the difference between freeze letters aimed at the first hop and letters aimed at where the money actually rests. Set it in the Railway worker env alongside the API-rate knobs. |
