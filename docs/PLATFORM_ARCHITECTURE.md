...
(brief / transfers / trace report / exhibit pack). ✅ **Live status via SSE**
— the detail page opens `GET /v2/traces/{id}/stream` (EventSource, JWT via
`?token=` since EventSource can't set headers; the server polls off the event
loop via `asyncio.to_thread` with a fresh short-lived connection per tick and
emits on change until terminal — the async-safety audit allowlist + baseline
were updated deliberately with an audited-non-blocking justification), falling
back to 4 s polling if EventSource errors. ✅ **D3 fund-flow graph** — a
"Fund-flow graph" button opens the engine's self-contained `interactive_graph.html`
via its presigned artifact URL (reuses the proven engine graph rather than
re-deriving one; a bespoke `/v2` graph-data endpoint is the only remaining
polish). The remaining fuller target (shadcn/TanStack component library) is the
next layer on this working base.
...
5. ✅ **DONE — Org invites + member management** — migration `039_org_invites` + `platform/store` DAO + `/v2/members` routes: list members, change role, remove (last-owner guarded), invite by email (seat-quota gated over members+pending, single-use hash-only token), list/revoke pending invites, and a PUBLIC `POST /v2/members/invites/accept` (token-gated: joins an existing user or creates the account). `web/` gets a Members page + `/invite` accept page. ✅ **Email verification + password reset DONE** — migration `041_user_tokens` (single-use hash-only tokens) + `POST /v2/auth/verify/{request,confirm}` and `POST /v2/auth/password/{reset-request,reset-confirm}`: reset-request is no-enumeration (always 202) and NEVER leaks the token in the response (emailed out-of-band), reset-confirm consumes atomically + rehashes. **Email delivery WIRED** — `platform/emailer.send_link_email` sends invite / verify / reset links via the engine's `worker._email.send_email` (Resend), best-effort (a missing `RESEND_API_KEY` just returns `success=False`, never raises; the invite/verify link is also returned to the authed caller, while the reset token is delivered ONLY by email).
6. ✅ **Prometheus metrics DONE** — the API process emits `recupero_platform_requests_total{endpoint,plan}` + `recupero_platform_signups_total` into the existing hardened `observability/metrics` registry, exposed at `GET /v2/metrics` (text exposition; the worker exposes its own `/metrics` on the health port). ✅ **OpenTelemetry tracing DONE (opt-in)** — `platform/tracing.py` instruments the FastAPI app with OTLP/HTTP export when `RECUPERO_OTEL_ENABLED` + the `[otel]` extra are present (no-op + never-raises otherwise; base install carries no otel dep). ✅ **Grafana dashboard** shipped at `infra/grafana/recupero-saas-dashboard.json` (trace-submissions-by-plan, signups, claims, stage-duration p95, freeze-letters, alerts). (Still TODO: structured `org_id` request logs.)
