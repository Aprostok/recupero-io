...
* **Operations:** Native TON outflows via v2 `getTransactions` (an outflow = an `out_msg` with positive `value`, nanoton/9-dec). Jetton outflows via v3 `jetton/transfers` (already-decoded source/destination/amount — no cell parsing). Jetton decimals: pinned canonical stables (USDT-TON → 6/tether) for certainty, else AUTHORITATIVE decimals from v3 `jetton/masters` (`jetton_content.decimals`, cached) + CoinGecko contract-resolution — never guessed; unresolvable → skipped. `is_contract` returns False (TON wallets are contracts but behave as accounts; protocol classification is label-driven).
* **Monitoring:** watch_tick supported — `_run_ton_chain` / `_snapshot_ton_one` value a watched TON wallet's native balance (v2 `getAddressInformation`) + Jetton balances (v3 `jetton/wallets`), pinned + authoritative-decimals jettons priced via CoinGecko. Optional `TONCENTER_API_KEY` lifts the rate.
...
* **Labels:** 1 issuer entry (USDT-TON → Tether, master verified live; freeze pathway). No CEX-deposit / hot-wallet entries yet — counterparties otherwise surface unlabeled.
* **Known gaps:** No TON CEX-deposit / hot-wallet labels seeded yet. Evidence receipt is explorer-anchored (no by-(lt,hash) raw receipt). Native-TON pagination is single-page (limit 100) — very high-volume addresses may truncate.
