...
For each entry in `all_significant_destinations`:
  ...
  * If it has `is_in_freeze_asks=true` AND `freeze_capability=no` → 🟪 TRACKED (e.g. DAI, wstETH — no issuer freeze pathway TODAY, but the funds are identified and still sitting there; monitor for movement, recoverable LATER if they move to a freezable venue or the perpetrator is identified). Still enumerate it so the customer sees the dollar amount.
  * If it has a `label_hint` matching a known MIXER (Tornado Cash, etc.) → ⬛ UNRECOVERABLE (funds mixed, no longer traceable). If it matches a known BRIDGE → 🟪 TRACKED when the bridged funds are traced to an identified destination (we follow cross-chain hops now); only ⬛ UNRECOVERABLE if the destination is genuinely unfollowable.
...
For DESTINATION_NOTES, use these emoji prefixes consistently:
  ...
  ⬛ UNRECOVERABLE — ONLY for funds that are genuinely GONE / untraceable: mixer deposits (Tornado Cash etc. — funds mixed), burn addresses, and bridges/paths whose destination we genuinely cannot follow. Briefly say WHY (e.g., "Tornado Cash — funds mixed, not traceable"). Do NOT mark identified, still-held funds UNRECOVERABLE just because they aren't freezable today — those are 🟪 TRACKED.
  🟪 TRACKED — for funds we've IDENTIFIED that are NOT freezable today but are still sitting at a known address and could be recovered LATER: addresses holding non-freezable assets (DAI/MakerDAO, wstETH, ETH-native — no issuer freeze pathway), and bridged funds traced to an identified destination. These are auto-enrolled in movement monitoring. Briefly say WHY + that it's monitored (e.g., "DAI is permissionless — no issuer freeze authority today; identified and monitored for movement"). NOTE: DEX aggregator routers (1inch, CoW GPv2, 0x, ParaSwap) and WETH9 are pass-through infrastructure — label those 🟧 INVESTIGATE (follow the swap output), not TRACKED.
